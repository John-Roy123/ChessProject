    ...
    protected Collection<Move> calculateKingCastles(final Collection<Move> playerLegals, final Collection<Move> opponentLegals) {
        ...
        if(this.playerKing.isFirstMove() && !this.isChecked()){
            ...
            if(!this.board.getTile(59).isFull() && !this.board.getTile(58).isFull() && !this.board.getTile(57).isFull()){
                ...
                if(rookTile.isFull() && rookTile.getPiece().isFirstMove() && Player.calculateAttacksOnTile(59, opponentLegals).isEmpty()
                && Player.calculateAttacksOnTile(58, opponentLegals).isEmpty() && rookTile.getPiece() instanceof Rook){
